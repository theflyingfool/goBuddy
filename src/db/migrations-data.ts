// Raw content of the migration SQL files, embedded as string constants so
// they can be applied without any filesystem access -- src/db/migrations.ts
// runs in the browser (Vite/jeep-sqlite+sql.js) as well as under Node
// (tests, scripts/build-dummy-db.ts), and drizzle-orm's own
// sqlite-proxy/migrator reads migration files via Node's fs module, which
// does not exist in a browser context -- discovered via this project's own
// e2e smoke test, which is exactly the kind of check that catches this.
// Generated once from the real migration files at
// src/db/migrations/000{0,1}_*.sql; those files are the source of truth --
// if a future migration changes them, regenerate this file to match
// (byte-for-byte, via .superpowers/sdd/gen-migrations-data.mjs or an
// equivalent script), never hand-edit the string constants below.
export const MIGRATION_0000_SQL = `-- Hand-edited after \`npm run db:generate\`: (1) restores REFERENCES clauses
...
export const MIGRATION_0001_SQL = `-- Hand-edited after \`npm run db:generate\`: (1) restores REFERENCES clauses
