...
| `schema/personal.ts` | Drizzle schema for personal tables — the sole input to `npm run db:generate`; source of truth for these tables' shape within the DB layer and Plan 2's query rewrite (`migrations.ts`, `completion-stats-sql.ts`, `reference-sync.ts`). **Not** unified with `types.ts` below — that stays the separate, hand-written source of truth for the rest of the app (deliberate, not yet reconciled). |
...
| `types.ts` | Hand-written camelCase TypeScript interfaces for both reference and personal tables — still the type source for `in-memory-store.ts`, `sqlite-repository.ts`, `field-groups.ts`, `defaults.ts`, and `repository.ts`; drives SQL, UI field groups, cascades, and the export format from one place. Independent of `schema/personal.ts`/`schema/reference.ts` above, not generated from them. |
